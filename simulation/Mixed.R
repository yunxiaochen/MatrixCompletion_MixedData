...
lik.bin <- function(M, data, Omega, tot){
  sum(Omega * data * M) - tot*sum(Omega * log((1+exp(M))))
}

lik.nor <- function(M, data, Omega){
  - sum(Omega * (data - M)^2/2)  
}
...
lik <- function(M, data, Omega, tot){
  ...
  lik1 = lik.bin(M1, data1, Omega1, tot)
  lik2 = lik.nor(M2, data2, Omega2)
...
grad.bin <- function(M0, data, Omega,tot){
  prob0 = 1/(1+exp(-M0))
  grad = (data - tot*prob0)/(tot*prob0*(1-prob0))*Omega
  grad
}

grad.nor <- function(M0, data, Omega){
  (data - M0)*Omega
}

grad<- function(M, data, Omega,tot){
  ...
  grad1 = grad.bin(M1, data1, Omega1,tot)
  grad2 = grad.nor(M2, data2, Omega2)
  cbind(grad1, grad2)
}
...
#============================================Functions for refinement for mixed data; Without splitting

grad.Theta.bin <- function(data, Omega, tot, A0, Theta0){
  M0 = Theta0%*% t(A0)
  prob0 = 1/(1+exp(-M0))
...
grad.Theta.nor <- function(data, Omega, A0, Theta0){
  M0 = Theta0%*% t(A0)
...
grad.Theta <- function(data, Omega, tot, A0, Theta0){
  ...
  grad1 = grad.Theta.bin(data1, Omega1, tot, A1, Theta0)
  grad2 = grad.Theta.nor(data2, Omega2, A2, Theta0)
  grad1 + grad2
}

grad2.Theta.bin <- function(data, Omega, tot, A0, Theta0){
  M0 = Theta0%*% t(A0)
  prob0 = 1/(1+exp(-M0))
...
grad2.Theta.nor <- function(data, Omega, A0, Theta0){
  grad2.M = Omega
  grad2.Theta = grad2.M %*% (A0^2)
  grad2.Theta
}
...
grad2.Theta <- function(data, Omega, tot, A0, Theta0){
  ...
  grad21 = grad2.Theta.bin(data1, Omega1, tot, A1, Theta0)
  grad22 = grad2.Theta.nor(data2, Omega2, A2, Theta0)
  grad21 + grad22
}
  
  
  
grad.A.bin <- function(data, Omega, tot, A0, Theta0){
  M0 = Theta0%*% t(A0)
  prob0 = 1/(1+exp(-M0))
  grad.M = (data - tot*prob0)*Omega
...
grad2.A.bin <- function(data, Omega, tot, A0, Theta0){
  M0 = Theta0%*% t(A0)
  prob0 = 1/(1+exp(-M0))
  grad2.M = tot*prob0 *(1-prob0)*Omega
...
grad.A.nor <- function(data, Omega, A0, Theta0){
  M0 = Theta0%*% t(A0) 
  grad.M = (data - M0) *Omega
  grad.A = t(grad.M) %*% Theta0
  grad.A
...
grad2.A.nor <- function(data, Omega, A0, Theta0){
  grad2.M = Omega
  grad2.A = t(grad2.M) %*% (Theta0^2)
  grad2.A
}
...
grad.A <- function(data, Omega, tot, A0, Theta0){
  ...
  grad1 = grad.A.bin(data1, Omega1, tot, A1, Theta0)
  grad2 = grad.A.nor(data2, Omega2, A2, Theta0)
  rbind(grad1, grad2)
}
...
grad2.A <- function(data, Omega, tot, A0, Theta0){
  ...
  grad21 = grad2.A.bin(data1, Omega1, tot, A1, Theta0)
  grad22 = grad2.A.nor(data2, Omega2, A2, Theta0)
  rbind(grad21, grad22)
...
lik.row.bin <- function(M, data, Omega, tot){
  rowSums(Omega * data * M) - tot*rowSums(Omega * log((1+exp(M))))
}

lik.row.nor <- function(M, data, Omega){
  - rowSums((Omega * (data - M)^2/2))
}
...
lik.row <- function(M, data, Omega, tot){
  ...
  lik.row.bin(M1, data1, Omega1, tot) + lik.row.nor(M2, data2, Omega2)
}


lik.col.bin <- function(M, data, Omega, tot){
  colSums(Omega * data * M) - tot*colSums(Omega * log((1+exp(M))))
}


lik.col.nor <- function(M, data, Omega ){
  - colSums((Omega * (data - M)^2/2))
}
...
lik.col <- function(M, data, Omega, tot){
  ...
  c(lik.col.bin(M1, data1, Omega1, tot), lik.col.nor(M2, data2, Omega2))
}
